...
import { X, Search, Loader2, Plus, ExternalLink, RefreshCw } from 'lucide-react';
        ...
        >
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => regenerateAssets('both')}
                disabled={isGeneratingLibrary}
                title={lang === 'cs' ? 'Znovu vyhledat videa a místa' : 'Regenerate Media & Library'}
                className={`p-2 rounded-xl transition-all flex items-center justify-center disabled:opacity-50 ${theme === 'dark' ? 'bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400' : 'bg-emerald-100 hover:bg-emerald-200 text-emerald-600'}`}
              >
                {isGeneratingLibrary ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}
              </button>
          ...
          <div className="flex gap-2 mb-4 overflow-x-auto pb-2 scrollbar-hide">
            {libraryTabs.map(tab => (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest flex-shrink-0 transition-all border ${activeTab === tab
