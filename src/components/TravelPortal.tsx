...
interface TravelPortalProps {
  ...
  // NEW PROPS
  regenerateAssets: (types: 'videos' | 'library' | 'both') => Promise<void>;
...
export const TravelPortal: React.FC<TravelPortalProps> = ({
  ...
  theme,
  setTheme,
  regenerateAssets,     // <--- Destructured new prop
  isGeneratingLibrary   // <--- Destructured new prop
}) => {
  ...
  // Auto-generate: triggered by Sara on the landing page via trigger_smart_itinerary_generation tool.
  const hasAutoGeneratedRef = useRef(false);
  useEffect(() => {
    if (hasAutoGeneratedRef.current) return;
    const shouldGenerate = localStorage.getItem('auto_generate_smart_itinerary') === 'true';
    if (shouldGenerate) {
      hasAutoGeneratedRef.current = true;
      const intensity = (localStorage.getItem('smart_itinerary_intensity') || 'balanced') as 'relaxed' | 'balanced' | 'packed';
      localStorage.removeItem('auto_generate_smart_itinerary');
      localStorage.removeItem('smart_itinerary_intensity');
      console.log('[TravelPortal] Auto-generating itinerary with intensity:', intensity);
      setTimeout(() => handleSmartGeneration(intensity), 1200);
    }
  ...
  const mergedSearchResults = useMemo(() => {
    ...
    (activeTrip?.libraryPois || []).forEach((p: any) => allKnownPois.set(p.id, p));
      ...
      <div className="flex flex-col-reverse md:flex-row h-screen w-full font-sans transition-colors duration-700 overflow-hidden bg-zinc-950">
        ...
        <div className="flex-1 relative h-1/2 md:h-full">
          ...
          <LibraryPanel
            ...
            regenerateAssets={regenerateAssets}         // <--- PASSED TO COMPONENT
            isGeneratingLibrary={isGeneratingLibrary}   // <--- PASSED TO COMPONENT
          />
        </div>
