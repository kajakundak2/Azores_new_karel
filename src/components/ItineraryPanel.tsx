...
import { Sparkles, MapPin, Instagram, RefreshCw, Loader2 } from 'lucide-react';
      ...
      <div className="mb-10 mt-2 overflow-hidden">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-4">
            ...
            {regenerateAssets && (
              <button
                onClick={() => regenerateAssets('videos')}
                disabled={isGeneratingLibrary}
                title={lang === 'cs' ? 'Znovu načíst videa' : 'Regenerate videos'}
                className={`text-[10px] transition-colors uppercase tracking-widest flex items-center gap-1.5 ${isGeneratingLibrary
                    ? 'text-zinc-600 cursor-not-allowed'
                    : 'text-zinc-500 hover:text-emerald-400'
                  }`}
              >
                {isGeneratingLibrary ? <Loader2 size={10} className="animate-spin" /> : <RefreshCw size={10} />}
                <span className="hidden sm:inline">{lang === 'cs' ? 'PŘENAHRÁT' : 'REGENERATE'}</span>
              </button>
            )}
            ...
            return displayVideos.map((v: any) => (
              <div key={v.id} className="relative flex-shrink-0 w-44 h-72 rounded-3xl overflow-hidden snap-center group border border-white/5 hover:border-emerald-500/50 transition-all duration-500 shadow-xl">
                <video
                  ...
                  loop
                  playsInline
                  crossOrigin="anonymous"  /* <-- FIX: Forces CORS headers for caching */
                  preload="metadata"       /* <-- FIX: Prevents aggressive buffer fetching */
