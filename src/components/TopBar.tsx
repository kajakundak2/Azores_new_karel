      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex flex-col min-w-[120px] max-w-[200px]">
          <input 
            ...
            className={`text-xs font-black uppercase tracking-tighter bg-transparent border-none p-0 outline-none focus:text-emerald-400 truncate ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}
          />
          <input 
            value={trip.destination || ''}
            placeholder={t('topbar_destination_placeholder')}
            onChange={(e) => onUpdate({ destination: e.target.value })}
            className={`text-[10px] font-bold bg-transparent border-none p-0 outline-none text-emerald-500/80 hover:text-emerald-500 focus:text-emerald-500 truncate`}
          />
        </div>
